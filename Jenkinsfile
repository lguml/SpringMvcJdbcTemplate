peline {
      ...
      stages {
        stage('Cloning Git') {
          steps {
           git branch: '**', credentialsId: 'githublguml', url: 'https://github.com/lguml/SpringMvcJdbcTemplate'
          }
        }
        stage('Build') {
           steps {
        ...
        stage('Building image') {
    	   steps{
    		  sh 'docker build . -t springmvcjdbctemplate:${BUILD_NUMBER}'  
    
            }
         }
        stage('tag image') {
          steps{
              sh 'docker tag springmvcjdbctemplate:${BUILD_NUMBER} lgumldocker/app:latest' 
            }
         }
        stage('push app --> dockerhub') {
         ...
         stage('Remove Unused app docker image') {
            steps{
                sh 'docker rmi springmvcjdbctemplate:$BUILD_NUMBER lgumldocker/app:latest'
            }
        }
